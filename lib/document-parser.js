...
export async function parsePDF(buffer) {
    try {
        console.log('PDF parse: Starting, buffer size:', buffer.length)
        const data = await pdf(buffer)
        console.log('PDF parse: Complete. Pages:', data.numpages, 'Text length:', data.text?.length || 0)

        if (!data.text || data.text.trim().length === 0) {
            return {
                success: false,
                error: `PDF contains no extractable text. This might be a scanned/image-based PDF (${data.numpages} pages). Please use a text-based PDF or OCR the document first.`,
                text: '',
                pages: data.numpages,
            }
        }
