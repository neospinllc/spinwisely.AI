...
export async function createUser(uid, data) {
    try {
        // Auto-grant admin access to specific email
        const isAdminEmail = data.email === 'info.spinwisely@gmail.com'

        await setDoc(doc(db, 'users', uid), {
            email: data.email,
            name: data.name,
            role: isAdminEmail ? 'admin' : 'user', // Auto-admin for specific email
            isAdmin: isAdminEmail, // Also set isAdmin flag
            termsAccepted: data.termsAccepted || false,
            termsAcceptedAt: data.termsAccepted ? Timestamp.now() : null,
            termsVersion: data.termsAccepted ? '1.0' : null,
            createdAt: Timestamp.now(),
            updatedAt: Timestamp.now(),
        })
        console.log(`User created successfully${isAdminEmail ? ' with admin access' : ''}`)
        return { success: true }
    } catch (error) {
...
export async function saveDocumentMetadata(documentData) {
    try {
        // Extract the id from documentData to use as the document ID
